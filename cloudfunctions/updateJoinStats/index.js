...
exports.main = async (event, context) => {
  ...
  if (res.data.length > 0) {
    ...
    await db.collection('user_carpool_stats').doc(userStats._id).update({
      data: {
        joinCount: userStats.joinCount + 1,           // 加入次数加 1
      totalParticipations: userStats.totalParticipations + 1, // 总参与次数加 1
      currentCarpoolCount: userStats.currentCarpoolCount + 1, // 当前拼车数量加 1
